...
######### Form functions for generating the html of the input fields ##########
	...
	function formCreator($canChange){
		...
		$button = we_html_button::create_button(we_html_button::SELECT, "javascript:we_cmd('we_users_selector','" . $idname . "','" . $textname . "','user',document.we_form.elements['" . $idname . "'].value,'setHot')");
...
<table class="default">
	...
	function formCopyDocument(){
		...
		$but = we_html_button::create_button(we_html_button::SELECT, "javascript:we_cmd('we_selector_document', document.we_form.elements['" . $idname . "'].value,'" . $this->Table . "','" . $idname . "','','copyDocumentSelect','','0','" . $this->ContentType . "',1);");
	...
	function formUserChooser($old_userID = -1, $width = '', $in_textname = '', $in_idname = ''){
		...
		return we_html_tools::htmlFormElementTable(self::htmlTextInput($textname, 30, $username, '', ' readonly', 'text', $width, 0), 'User', 'left', 'defaultfont', we_html_element::htmlHidden($idname, $userid), we_html_button::create_button(we_html_button::SELECT, "javascript:we_cmd('we_users_selector','document.we_form.elements['" . $idname . "'].value','" . $textname . "','user')"));
	}
...
# private ###################
	...
	protected function i_saveContentDataInDB(){
		...
		$ret = $data ?
			$this->DB_WE->query('REPLACE INTO ' . CONTENT_TABLE . ' (ID,Dat,BDID,DID,Name,Type,nHash,DocumentTable) VALUES ' . implode(',', $data)) :
			true;
		if(!$ret){
			return false;
		}
		...
		if($replace){
			$this->DB_WE->query('DELETE FROM ' . CONTENT_TABLE . ' WHERE DID=' . $this->ID . ' AND DocumentTable="' . $this->DB_WE->escape(stripTblPrefix($this->Table)) . '" AND ID IN(' . implode(',', $replace) . ')');
		}
		return true;
	}
...
// overwrite
	...
	protected function getNavigationFoldersForDoc(){
		...
		$queries = ['( (Selection="' . we_navigation_navigation::SELECTION_STATIC . '" OR IsFolder=1) AND SelectionType="' . we_navigation_navigation::STYPE_DOCLINK . '" AND LinkID=' . intval($this->ID) . ')',];
		...
		if($category){
			//FIXME: query should use ID, not parentID
			$queries[] = '(Selection="' . we_navigation_navigation::SELECTION_DYNAMIC . '" AND DynamicSelection="' . we_navigation_navigation::DYN_DOCTYPE . '" AND (FIND_IN_SET("' . implode('",Categories) OR FIND_IN_SET("', $category) . '",Categories) ) )';
		}
